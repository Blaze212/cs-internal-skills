...
def _cell_value(key: str, row: dict) -> str:
    ...
    if key in ("original_url", "png_url", "svg_url"):
        ...
        if not label:
            ...
            title = row.get("title", "")
            suffixes = {"original_url": ".png", "png_url": "-redacted.png", "svg_url": "-editable.svg"}
...
def append_row(sheet_id: str, row: dict) -> list[str]:
    ...
    # Build the row in column order and track unknown values
    # Note: Column order doesn't matter — we match columns by header name, not position
    values = []
    unknowns = []
    for header in headers:
        key = _resolve_key(header)
        cell_val = _cell_value(key, row) if key else ""
        values.append(cell_val)
        if key and cell_val and "unknown" in cell_val.lower():
            unknowns.append(f"{header}: {cell_val}")
...
def main() -> int:
    ...
    unknowns = append_row(sheet_id, row)
    if unknowns:
        print(f"⚠️  Found {len(unknowns)} unknown value(s):", file=sys.stderr)
